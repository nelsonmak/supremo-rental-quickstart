...
function CarCheckOut({ bookingDetails }) {
  ...
  useEffect(() => {
    fetch("http://140.238.10.184/car-service-redis/cars" + `/${id}`)
  ...
  useEffect(() => {
    ...
    fetch(`http://140.238.10.184/order-service/user-orders?userid=${getUserId}`)
      .then((response) => response.json())
      .then((data) => {
  ...
  useEffect(() => {
    fetch(`http://140.238.10.184/user-service-redis/users/${getUserId}`)
      .then((response) => response.json())
      .then((data) => {
        // console.log("from http://146.56.171.43:8081 ", id);
        setUserData(data);
  ...
  const handleCheckout = async () => {
    ...
    const response = await fetch("http://140.238.10.184/order-service/create-order", {
      method: 'POST',
      headers: {
